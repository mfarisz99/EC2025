...
# List of icons to assign to cities
icons_list = ["♕", "♖", "♗", "♘", "♙", "♔", "♚", "♛", "♜", "♝"]
...
# Collect city data from the user
for i in range(n_cities):
    name = input(f"Enter the name of city {i + 1}: ")
    x_coord = float(input(f"Enter the X coordinate for {name}: "))
    y_coord = float(input(f"Enter the Y coordinate for {name}: "))
...
# Dynamically assign icons to each city
city_icons = {name: icons_list[i % len(icons_list)] for i, name in enumerate(cities_names)}

# Print the city icons dictionary to verify
print("City Icons:", city_icons)
